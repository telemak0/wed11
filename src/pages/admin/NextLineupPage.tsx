...
export const NextLineupPage = () => {
    ...
    const handlePlayerClick = async (player: Player) => {
        ...
        // Save to DB
        try {
            await matchService.updateLineup(
                match.id, 
                Array.from(newWhite), 
                Array.from(newRed)
            );
    ...
    const handleRemovePlayer = async (playerId: string) => {
        ...
        if (removedFromWhite || removedFromRed) {
             ...
             if (removedFromWhite) {setActiveTeam('white');}
             if (removedFromRed) {setActiveTeam('red');}
    ...
    const handleCloseMatch = async () => {
        ...
        if (whiteScore === null || redScore === null) {return;}
    ...
    if (loading) {return <div className="p-8 text-center">{t('nextLineup.loading')}</div>;}
        ...
        return (
            <div className="p-8 text-center">
                ...
                >
                 ...
                 <div className="mt-6">
                    <Link to="/admin/dashboard" className="text-gray-600 hover:underline">{t('nextLineup.backToDashboard')}</Link>
                </div>
            </div>
        );
    }
    ...
    const teamWhitePlayers = Array.from(selectedWhiteIds).map(id => allPlayers.find(p => p.id === id)).filter(Boolean) as Player[];
    const teamRedPlayers = Array.from(selectedRedIds).map(id => allPlayers.find(p => p.id === id)).filter(Boolean) as Player[];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500">{t('nextLineup.matchDate')}: {match.date.toDate().toLocaleDateString()}</p>
                    {match.status === 'completed' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mt-1">{t('nextLineup.completed')}</span>}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                        <label className="text-sm font-semibold text-gray-500 uppercase tracking-wide block mb-3">{t('nextLineup.selectActiveTeam')}</label>
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 h-[500px] overflow-y-auto">
                        <h3 className="text-lg font-medium text-gray-900 mb-4 sticky top-0 bg-white pb-2 border-b">{t('nextLineup.availablePlayers')}</h3>
                        
                        {/* Active Players */}
                        <div className="mb-6">
                            <h4 className="text-md font-semibold text-gray-700 mb-2">{t('nextLineup.activePlayers')}</h4>
                            <div className="flex flex-wrap gap-2">
                                {allPlayers.filter(p => p.type === 'active').map(player => {
                                    const isSelected = selectedWhiteIds.has(player.id) || selectedRedIds.has(player.id);
                                    return (
                                        <button
                                            key={player.id}
                            ...
                            {allPlayers.filter(p => p.type === 'active').length === 0 && <p className="text-gray-500 italic text-sm">{t('nextLineup.noActivePlayers')}</p>}
                        </div>

                        {/* Occasional Players */}
                        <div>
                            <h4 className="text-md font-semibold text-gray-700 mb-2">{t('nextLineup.occasionalPlayers')}</h4>
                            <div className="flex flex-wrap gap-2">
                                {allPlayers.filter(p => p.type === 'occasional').map(player => {
                                    const isSelected = selectedWhiteIds.has(player.id) || selectedRedIds.has(player.id);
                                    return (
                                        <button
                                            key={player.id}
                            ...
                            {allPlayers.filter(p => p.type === 'occasional').length === 0 && <p className="text-gray-500 italic text-sm">{t('nextLineup.noOccasionalPlayers')}</p>}
                        </div>

                        {allPlayers.length === 0 && <p className="text-gray-500 italic">{t('nextLineup.noPlayersFound')}</p>}
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center">
                    <div className="bg-gray-900 p-6 rounded-xl shadow-inner w-full flex flex-col items-center">
                        <h3 className="text-white text-lg font-bold mb-4 uppercase tracking-widest">{t('nextLineup.lineup')}</h3>
                        <PitchView 
